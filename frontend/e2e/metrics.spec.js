...
test.describe('Metrics API', () => {
  const baseURL = process.env.API_URL || 'http://localhost:8003';
  ...
  test('GET /api/metrics/summary returns statistics', async ({ request }) => {
  ...
  test('GET /api/metrics/repository/{repo} returns specific repo metrics', async ({ request }) => {
  ...
  test('GET /api/metrics/repository/{repo} returns 404 for non-existent repo', async ({ request }) => {
    const response = await request.get(`${baseURL}/api/metrics/repository/nonexistent/fake-repo`);
    
    expect(response.status()).toBe(404);
  });

  test('GET /api/scans/recent returns scan history', async ({ request }) => {
