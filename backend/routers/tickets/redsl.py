...
async def _run_redsl(redsl: RedslClient, data: RedslAutoPRRequest) -> _RedslResult:
    ...
    if data.dry_run:
        ...
        decisions = (
            decide_result.get("decisions", [])
            if isinstance(decide_result, dict)
            else decide_result
        )
        ...
        applied = decisions
        modified_files = [
            d.get("target_path", "") for d in applied if d.get("target_path")
        ]
    ...
    if cycle_result.get("proposals_applied", 0) == 0 and not modified_files:
        return _RedslResult(
            [], [], decisions_count, "no_changes", "No refactoring changes applied"
        )
...
def _build_redsl_response(
    ...
    *,
    ...
    if result.status in ("no_targets", "no_changes"):
        ...
        update_ticket_redsl_results(
            db, ticket_id, result.applied, result.modified_files
        )
...
async def _create_pr_for_ticket(
    ...
    try:
        ...
        branch = (
            f"ticket-{ticket_id[:8]}-{datetime.now(timezone.utc).strftime('%Y%m%d')}"
        )
        ...
        if not patches:
            mark_ticket_error(
                get_db().__next__(), ticket_id, "No patchable files found"
            )
            return
