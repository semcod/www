...
async def _commit_patches(
    repo: str, patches: list, branch: str, proposal_type: str, fix_id: str, token: str
) -> None:
...
async def create_auto_pr(
    ...
    try:
        ...
        await _commit_patches(
            body.repo, body.patches, branch, body.proposal_type, fix_id, token
        )

        rollback_reason = _check_rollback(body.repo)
        if rollback_reason:
            _, score_after_cmp = _score_improved(body.repo)
            return await _handle_rollback(
                body.repo,
                body.proposal_type,
                fix_id,
                body.llm_prompt,
        ...
        _, score_after_cmp = _score_improved(body.repo)
        pr_title = (
            f"[Semcod] Auto-fix: {body.proposal_type.replace('_', ' ')} [{fix_id}]"
        )
        ...
        pr_url = await PRCreator.create_pr(
            body.repo, branch, default_branch, pr_title, pr_body, token
        )
...
async def _run_redsl_refactor(
    redsl: RedslClient, project_path: str, max_actions: int, dry_run: bool
) -> list:
...
async def _commit_redsl_decisions(
    repo: str,
    decisions: list,
    project_path: str,
    branch: str,
...
async def create_redsl_auto_pr(
    ...
    try:
        decisions = await _run_redsl_refactor(
            redsl, body.project_path, body.max_actions, body.dry_run
        )
        if not decisions:
            return RedslRefactorResult(
                status="no_decisions", redsl_available=True, decisions_count=0
            )
        ...
        pr_body = _build_redsl_pr_body(
            body.proposal_type, fix_id, decisions, committed_files
        )
        pr_url = await PRCreator.create_pr(
            body.repo, branch, default_branch, pr_title, pr_body, token
        )
        ...
        return RedslRefactorResult(
            status="failed", redsl_available=True, error=str(exc)
        )


def _build_redsl_pr_body(
    proposal_type: str, fix_id: str, decisions: list, files: set
) -> str:
